...
export default function Properties() {
  ...
  const getPropertyBadges = (prop) => {
    const badges = [];
    if (prop.averageRating >= 4.8 && (prop.totalReviews >= 2 || prop.price > 5000)) {
      badges.push({ text: 'Guest Favourite', type: 'gold' });
    } else if (prop.price && prop.price <= 2500) {
      badges.push({ text: 'Best Value', type: 'emerald' });
    } else if (prop.price && prop.price >= 6000) {
      badges.push({ text: 'Premium Sanctuary', type: 'champagne' });
    }
    if ((prop.verificationStatus === 'approved' || !prop.verificationStatus) && badges.length < 2) {
      badges.push({ text: 'Verified Stay', type: 'verified' });
    }
  ...
  // Client-side refinement for Amenities & Guests
  const filteredProperties = properties.filter((prop) => {
    // 1. Amenities filter
    if (selectedAmenities.length > 0) {
      const propAmenities = (prop.amenities || []).map((a) => a.toLowerCase());
      const hasAll = selectedAmenities.every((selected) =>
        propAmenities.some((pa) => pa.includes(selected.toLowerCase()) || selected.toLowerCase().includes(pa))
      );
    ...
    if (guests && Number(guests) > 0) {
      const minBedsNeeded = Math.ceil(Number(guests) / 2);
      if (prop.bedrooms && prop.bedrooms < minBedsNeeded) return false;
    }
    ...
    <div className="min-h-[calc(100vh-5rem)] bg-[#fbfbf9] dark:bg-[#121214] py-10 sm:py-14 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        >
          ...
          {/* Secondary Filter Row: Dates & Guests & Verification */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 pt-1">
            {/* Check-in Date */}
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-[#71717a] dark:text-[#a1a1aa] mb-1" htmlFor="filter-checkin">
                Check-in Date
              </label>
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 bg-[#fbfbf9] dark:bg-[#141417] border border-[#e5e0d8] dark:border-[#27272a] rounded-xl text-xs font-semibold text-[#18181b] dark:text-[#fbfbf9] focus:outline-none focus:ring-1 focus:ring-[#b58d59] cursor-pointer"
                />
              </div>
            </div>
            ...
            {/* Check-out Date */}
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-[#71717a] dark:text-[#a1a1aa] mb-1" htmlFor="filter-checkout">
                Check-out Date
              </label>
              <div className="relative">
                <input
            ...
            {/* Guests */}
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-[#71717a] dark:text-[#a1a1aa] mb-1" htmlFor="filter-guests">
                Guests
              </label>
              <select
                id="filter-guests"
          ...
          {/* Tertiary Filter Row: Amenities Multi-Select Filter Pills */}
          <div className="space-y-2 pt-2 border-t border-[#f4f0e8] dark:border-[#27272a]">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#71717a] dark:text-[#a1a1aa] flex items-center gap-1.5">
              ...
              {selectedAmenities.length > 0 && (
                <button
                  ...
                  className="text-[10px] font-semibold text-[#71717a] hover:text-[#18181b] dark:hover:text-white"
                >
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer ${
                      isSelected
                        ? 'bg-[#18181b] dark:bg-[#d4b996] text-white dark:text-[#18181b] shadow-sm'
                        : 'bg-[#f4f0e8] dark:bg-[#27272a] text-[#52525b] dark:text-[#a1a1aa] hover:bg-[#ede7dc] dark:hover:bg-[#3f3f46]'
                    }`}
                  >
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-[#1c1c20] rounded-[2rem] border border-[#e5e0d8] dark:border-[#27272a] p-12 text-center max-w-lg mx-auto shadow-editorial space-y-4">
            ...
            <p className="text-xs text-[#71717a] dark:text-[#a1a1aa]">
              Try changing your dates, location, or filters to discover available stays.
            </p>
            <button
              onClick={handleClearFilters}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between text-xs text-[#71717a] dark:text-[#a1a1aa] font-semibold px-1">
              <span>Showing {filteredProperties.length} {filteredProperties.length === 1 ? 'property' : 'properties'} on Map</span>
              {hasActiveFilters && (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredProperties.map((property) => {
                ...
                const detailUrl = `/properties/${property._id}${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
                    ...
                    <div>
                      ...
                      <div className="relative h-64 bg-[#f4f0e8] dark:bg-[#27272a] overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4 flex flex-col gap-1.5 items-start">
                          <span className="bg-white/90 dark:bg-[#18181b]/90 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider text-[#18181b] dark:text-[#fbfbf9] shadow-sm border border-[#e5e0d8] dark:border-[#27272a]">
                            {property.propertyType}
                          </span>
                          {badges.map((b, idx) => (
                            <span
                              key={idx}
                              className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider shadow-sm border backdrop-blur-md ${
                                b.type === 'gold'
                                  ? 'bg-[#d4b996]/90 text-[#18181b] border-[#c5a880]'
                                  : b.type === 'emerald'
                                  ? 'bg-emerald-900/85 text-emerald-100 border-emerald-700'
                                  : b.type === 'champagne'
                                  ? 'bg-[#b58d59]/90 text-white border-[#8c6b3e]'
                                  : 'bg-white/90 dark:bg-[#18181b]/90 text-emerald-700 dark:text-emerald-400 border-emerald-300 dark:border-emerald-800'
                              }`}
                            >
                              {b.text}
                            </span>
                      ...
                      <div className="p-6 space-y-3">
                        ...
                        <div className="pt-2 flex items-center gap-4 text-xs font-medium text-[#71717a] dark:text-[#a1a1aa] border-t border-[#f4f0e8] dark:border-[#27272a]">
                          ...
                          {property.amenities && property.amenities.length > 0 && (
                            <div className="flex items-center gap-1.5 ml-auto text-[11px] text-[#8c827a] dark:text-[#a1a1aa]">
                              <Sparkles className="w-3 h-3 text-[#b58d59]" />
