    ...
    return (
      <div className="min-h-screen bg-[#fafafa] py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-[36px] border border-slate-200/80 p-8 sm:p-14 text-center shadow-sm space-y-6">
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
              <Link
                to="/properties"
                className="inline-flex items-center gap-2 px-6 py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-2xl shadow-lg transition-all cursor-pointer"
              >
              ...
              <Link
                to="/"
                className="inline-flex items-center gap-2 px-6 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-2xl transition-all cursor-pointer"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 bg-white p-7 sm:p-9 rounded-[32px] border border-slate-200/80 shadow-sm">
          <div>
            <Link
              to="/properties"
              className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-slate-900 transition-colors mb-2 group"
            >
          ...
          <div className="flex items-center gap-3">
            {properties.length < maxLimit && (
              <Link
                to="/properties"
                className="inline-flex items-center gap-1.5 px-5 py-3 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-2xl transition-all shadow-sm cursor-pointer"
              >
                <Plus className="w-4 h-4" />
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-3 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs rounded-2xl border border-rose-200 transition-colors cursor-pointer"
            >
        ...
        <div className="bg-white rounded-[32px] border border-slate-200/90 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.03)] overflow-x-auto">
          <table className="w-full border-collapse min-w-[750px]">
            {/* 1. PROPERTY HEADER CARDS */}
            <thead>
              <tr className="border-b border-slate-200/80 bg-slate-50/50">
                <th className="p-6 text-left w-56 min-w-56 text-xs font-bold text-slate-400 uppercase tracking-wider align-top">
                  Property Highlights
                </th>
                {properties.map((property) => (
                  <th
                    key={property._id}
                    className="p-6 text-left w-72 min-w-72 align-top border-l border-slate-100"
                  >
                    <div className="space-y-3.5">
                      ...
                      <div className="relative h-44 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200">
                        ...
                        {/* Property Type Pill */}
                        <span className="absolute top-2.5 left-2.5 bg-white/90 backdrop-blur-md px-2.5 py-0.5 rounded-full text-[10px] font-bold text-slate-900 shadow-sm border border-white/50">
                      ...
                      {(property.verificationStatus === 'approved' || !property.verificationStatus) && (
                        <div>
                          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200/90 px-2.5 py-0.5 rounded-full">
                      ...
                      <Link
                        ...
                        className="block font-bold text-slate-900 text-sm line-clamp-2 hover:text-indigo-600 transition-colors"
                      >
                        {property.title}
                      </Link>
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-1">
                        <Link
                          to={`/properties/${property._id}`}
                          className="px-3 py-2 text-center text-xs font-bold rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-700 transition-colors"
                        >
                          Details
                        </Link>
                        <Link
                          to={`/properties/${property._id}/book`}
                          className="px-3 py-2 text-center text-xs font-bold rounded-xl bg-slate-900 hover:bg-slate-800 text-white shadow-sm transition-colors"
                        >
                          Book Stay
                        </Link>
                      </div>
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              ...
              <tr className="hover:bg-slate-50/50 transition-colors">
                <td className="p-5 font-bold text-slate-700 uppercase tracking-wider bg-slate-50/30">
                  Monthly Rent
                </td>
                {properties.map((p) => {
                  const isLowest = lowestRent && Number(p.price) === lowestRent;
                  return (
                    <td key={p._id} className="p-5 border-l border-slate-100">
              ...
              <tr className="hover:bg-slate-50/50 transition-colors">
                ...
                {properties.map((p) => (
                  <td key={p._id} className="p-5 border-l border-slate-100 font-semibold text-slate-800">
              ...
              <tr className="hover:bg-slate-50/50 transition-colors">
                ...
                {properties.map((p) => (
                  <td key={p._id} className="p-5 border-l border-slate-100 text-slate-700">
                  ...
                  <td key={p._id} className="p-5 border-l border-slate-100">
                    {p.totalReviews > 0 ? (
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1 text-amber-600 font-bold bg-amber-50 px-2 py-0.5 rounded-lg border border-amber-200">
              ...
              <tr className="bg-slate-900 text-white">
                <td
                  ...
                  className="p-4 text-[11px] font-bold uppercase tracking-widest text-slate-200"
                >
              ...
              {standardAmenities.map((amenity) => {
                ...
                return (
                  <tr key={amenity.name} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-5 font-bold text-slate-700 flex items-center gap-2 bg-slate-50/30">
                    ...
                    {properties.map((p) => {
                      ...
                      return (
                        <td key={p._id} className="p-5 border-l border-slate-100">
                          {included ? (
                            <span className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200/90 px-2.5 py-1 rounded-xl">
                              <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                              <span>Included</span>
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-400 bg-slate-50 px-2.5 py-1 rounded-xl">
